...
if (file_exists('tcpdf/tcpdf.php')) {
    ...
    $locations = $devicesModel->getDeviceAllLocations($deviceId, $page);
    ...
    $pdf->SetHeaderData("../../../img/logo.png", 15, $deviceDescription."'s Locations (".$date." GMT)",
    ...
    $output = $output . "<tr>
                            ...
                            <td width=\"70%\"><strong>Closest Street Address</strong></td>
                         </tr>";
    $lonLatArray = array();
    foreach($locations as $location)
    {
        $id = $location['id'];
        $date = substr($location['date'], 0, 10);
        $date = gmdate("F j, Y, g:i a", $date) ." GMT";
        $longitude = $location['longitude'];
        $latitude = $location['latitude'];
        if($lonLatArray[$longitude.$latitude]) $address = $lonLatArray[$longitude.$latitude];
        else
        {
    ...
    ob_end_clean();
    $pdf->Output($deviceDescription.'_Locations.pdf', 'I');
}
